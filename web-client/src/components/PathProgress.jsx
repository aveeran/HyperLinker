    ...
    if (isChromeExtension) {
      chrome.storage.local.get(
        ...
        (result) => {
          ...
          const storedEndGameInfo =
            result[utils.END_GAME_INFO] || utils.defaultEndGameInfo;
          const ended = storedEndGameInfo.ended || false; // TODO: rework the data structure

          const storedGameInformation = ended
            ? storedEndGameInfo.singleplayerGameInformation
            : result[utils.SINGLEPLAYER_GAME_INFORMATION] ||
              utils.defaultGameInformation;

          const storedGameProperties = ended
            ? storedEndGameInfo.singleplayerGameProperties
          ...
          let endIndex =
            storedGameProperties.path?.length ||
            storedGameInformation.freePath?.length;
          endIndex--;
        ...
        if (areaName === "local") {
          ...
          if (
            ...
            const storedGameProperties =
              storedEndGameInfo.singleplayerGameProperties;
            const storedCustomizations =
              storedEndGameInfo.singleplayerCustomizations;
      ...
      <div className="flex flex-col gap-4 p-2">
        {(isDirected || !isPath) &&
          path
            ...
            ${
              rowIndex % 2 == 0
                ? visited.includes(path[2]?.link) &&
                  visited.includes(path[3]?.link)
                  ? "bg-green-500"
                  : "bg-gray-300"
                : visited.includes(path[5]?.link) &&
                  visited.includes(path[6]?.link)
                ? "bg-green-500"
                : "bg-gray-300"
            }    
            ...
            .flatMap((row, rowIndex, rows) => [
              ...
              rowIndex < rows.length - 1 && (
                ...
                >
                  ...
                  ${
                    rowIndex % 2 == 0
                      ? visited.includes(path[2]?.link) && visited.includes(path[3]?.link)
                        ? "bg-green-500"
                        : "bg-gray-300"
                      : visited.includes(path[5]?.link) && visited.includes(path[6]?.link)
                      ? "bg-green-500"
                      : "bg-gray-300"
                  }    
                  ${rowIndex % 2 == 0 ? "mr-5" : "ml-5"}
